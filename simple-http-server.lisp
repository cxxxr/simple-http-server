    ...
    (let ((i 0))
      (labels ((type ()
                 (let ((pos (position #\/ string)))
                   (setf type (subseq string 0 pos))
                   (setf i (1+ pos))))
               (subtype ()
                 (let ((pos (position #\; string :start i)))
                   (setf subtype (string-right-trim " " (subseq string i pos)))
                   (setf i (if pos (1+ pos) (length string)))))
               (eosp ()
                 (not (< i (length string))))
               (skip-ws ()
                 (loop :while (< i (length string))
                       :do (if (char= #\space (aref string i))
                               (incf i)
                               (return))))
               (parameter ()
                 (skip-ws)
...
(defun maybe-set-post-parameters (request)
  (when-let (content-type (cdr (assoc "Content-Type" (request-fields request) :test #'string=)))
    (multiple-value-bind (type subtype parameters) (parse-media-type content-type)
